...
public class LabsystecBlogService : IBlogService
{
    ...
    public async Task<OperationResult<PostDto?>> CreatePostAsync(PostDto dto)
    {
        try
        {
        ...
        {
            return new OperationResult<PostDto?>(false, "An error occurred during creating post", exception: ex);
        }
    }
